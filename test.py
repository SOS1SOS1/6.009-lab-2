...
def compare_greyscale_images(im1, im2):
    ...
    for ix, (i, j) in enumerate(zip(im1['pixels'], im2['pixels'])):
        assert i == j, 'Incorrect value at location %s (differs from expected by %s) im1 = %s im2 = %s' % (ix, abs(i-j), i, j)
...
def compare_color_images(im1, im2):
    ...
    for ix, (i, j) in enumerate(zip(im1['pixels'], im2['pixels'])):
        if i != j:
            assert False, 'Incorrect value at location %s (differs from expected by %s) i: %s j: %s' % (ix, tuple(abs(i[t]-j[t]) for t in {0,1,2}), i, j)
